    ...
    if system_name == "Windows":
        ...
        try:
            ...
            subprocess.run([
                "powershell",
                "-Command",
                "Start-Process notepad.exe -ArgumentList '{}' -Verb RunAs".format(hosts_path)
            ])
        except Exception as e:
            print("Failed to open hosts file as Administrator:", e)
    
    elif system_name == "Linux":
        hosts_path = "/etc/hosts"
        try:
            subprocess.run(["sudo", "xdg-open", hosts_path])
        except Exception as e:
            print("Failed to open hosts file (sudo required):", e)
    
    elif system_name == "Darwin":  # macOS
        hosts_path = "/etc/hosts"
        try:
            subprocess.run(["sudo", "open", "-a", "TextEdit", hosts_path])
        except Exception as e:
            print("Failed to open hosts file (sudo required):", e)
...
def block_website():
    ...
    weburl = entry_url.get().strip()

    if weburl.startswith("http://") or weburl.startswith("https://"):
        weburl = weburl.split("//", 1)[1]  # Remove only the first occurrence of //

    hosts_path = get_hosts_path()
    if not hosts_path:
        return

    entry = "127.0.0.1 " + weburl + "\n"
    entry += "127.0.0.1 " + "www." + weburl + "\n"
    try:
